...
def unrolled_cython_validate(str tax_id):
    if len(tax_id) != 11:
        return False
    cdef cython.int x[11]
    for i in range(11):
        x[i] = ord(tax_id[i]) - ord('0')
    cdef cython.int m = (
        (10*x[0] + 9*x[1] + 8*x[2] + 7*x[3] + 6*x[4]
...
def unrolled_regex_cython_validate(str tax_id):
    if len(tax_id) != 11:
        return False
    cdef cython.int x[11]
    for i in range(11):
...
_all_eq_pat = re.compile(r'^(\d)\1{10}$') #^(\d)\1+$')
